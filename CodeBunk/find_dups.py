...
# runs in time O(n).
"""
Given an array of length n containing integers from 1 to n-1, find any duplicate element.
...
def find_duplicate(A):
    n = len(A)
    for i in xrange(n):
        if A[i] > 0: #only cycle if not cycled before
            index = A[i] - 1
            while True: #cycle through A[A[A[i]]] till we hit a negative number or the original i
                if A[index] < 0:
                    A = map(abs,A) #restore array
                    return A[index]
                A[index] *= -1
                index = abs(A[index]) - 1
                if index == i: 
...
print find_duplicate( [1,3,1,2,2,1])
print find_duplicate( [1,3,2,4])